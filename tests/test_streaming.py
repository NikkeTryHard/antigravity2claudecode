"""
Tests for streaming conversion in antigravity2claudecode.
...
from antigravity2claudecode.helpers import remove_nulls_for_tool_input
from antigravity2claudecode.streaming import (
    _sse_event,
    _StreamingState,
...
class TestStreamingThinkingDisabled:
    ...
    async def test_thinking_stripped_when_disabled(self):
        ...
        lines = [
            make_antigravity_sse_data([{"thought": True, "text": "Secret thinking..."}]),
    ...
    async def test_thinking_only_response_handled(self):
        ...
        lines = [
            make_antigravity_sse_data([{"thought": True, "text": "Just thinking..."}], "STOP"),
        ]
...
class TestStreamingEdgeCases:
    ...
    async def test_stop_reason_tool_use(self):
        ...
        lines = [
            make_antigravity_sse_data([{"functionCall": {"name": "test", "args": {}}}], "STOP"),
        ]
...
@pytest.mark.asyncio
class TestStreamingEdgeCasesExtended:
    """Extended edge case tests for additional coverage"""

    async def test_invalid_initial_input_tokens(self):
        """Invalid initial_input_tokens should fall back to 0"""
        lines = [
            make_antigravity_sse_data([{"text": "Hello"}], "STOP"),
        ]
        ...
        async for event in antigravity_sse_to_anthropic_sse(
            ...
            model="test",
            message_id="msg_123",
            initial_input_tokens="invalid",  # type: ignore - testing invalid input
        ):
            events.append(event)
    ...
    async def test_non_dict_usage_metadata_in_response(self):
        """Non-dict usageMetadata in response should be handled"""
        data = {
            "response": {
                "candidates": [
                    {
    ...
    async def test_non_dict_usage_metadata_in_candidate(self):
        """Non-dict usageMetadata in candidate should be handled"""
        data = {
            "response": {
                "candidates": [
                    {
                        "content": {"parts": [{"text": "Hello"}]},
                        "finishReason": "STOP",
                        "usageMetadata": [1, 2, 3],  # Not a dict
                    }
                ],
            }
        }
    ...
    async def test_candidate_usage_preferred_when_more_complete(self):
        """Candidate usageMetadata should be used if more complete than response"""
        data = {
            "response": {
                "candidates": [
                    {
                ...
                "usageMetadata": {"promptTokenCount": 150},  # Less complete
            }
        }
        lines = [f"data: {json.dumps(data)}"]
        ...
        events_str = b"".join(events).decode("utf-8")
        # Should use candidate's 200 input tokens
        assert '"input_tokens":200' in events_str

    async def test_thinking_signature_handling(self):
        """Thinking signature should be handled correctly"""
        lines = [
            make_antigravity_sse_data([{"thought": True, "text": "Thinking..."}]),
            make_antigravity_sse_data(
                [{"thought": True, "text": " more", "thoughtSignature": "sig123"}]
            ),
            make_antigravity_sse_data([{"text": "Done."}], "STOP"),
        ]
        ...
        events_str = b"".join(events).decode("utf-8")
        assert "sig123" in events_str
        assert "signature_delta" in events_str

    async def test_empty_response_parts(self):
        """Empty parts list should be handled gracefully"""
        data = {
            "response": {
                "candidates": [
                    {
    ...
    async def test_null_candidates_handled(self):
        """Null/missing candidates should be handled"""
        data = {"response": {"candidates": None}}
        lines = [f"data: {json.dumps(data)}"]
    ...
    async def test_non_dict_part_skipped(self):
        ...
        data = {
            "response": {
                "candidates": [
                    {
                        "content": {"parts": ["string part", {"text": "Valid"}]},
                        "finishReason": "STOP",
                    }
                ],
            }
...
class TestCredentialManagerIntegration:
    ...
    async def test_credential_manager_called_on_success(self):
        """Credential manager should be called on successful response"""
        recorded_calls = []
        ...
        assert recorded_calls[0]["credential_name"] == "test_cred"
        assert recorded_calls[0]["success"] is True
        assert recorded_calls[0]["is_antigravity"] is True


# Run tests with: python -m pytest tests/test_streaming.py -v
